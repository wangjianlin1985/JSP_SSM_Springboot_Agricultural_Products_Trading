...
public class ExportExcelUtil {
	...
	public void exportExcel(String rootPath,String title, String[] headers,
		...
		// 声明一个工作薄
		HSSFWorkbook workproduct = new HSSFWorkbook();
		// 生成一个表格
		HSSFSheet sheet = workproduct.createSheet(title);
		// 设置表格默认列宽度为15个字节
		sheet.setDefaultColumnWidth((short) 15);
		// 生成一个样式
		HSSFCellStyle style = workproduct.createCellStyle();
		// 设置这些样式
		...
		// 生成一个字体
		HSSFFont font = workproduct.createFont();
		...
		style.setFont(font);
		// 生成并设置另一个样式
		HSSFCellStyle style2 = workproduct.createCellStyle();
		...
		// 生成另一个字体
		HSSFFont font2 = workproduct.createFont();
		...
		// 设置注释制造商，当鼠标移动到单元格上是可以在状态栏中看到该内容.
			...
			for (short i = 0; i < t.length; i++) {
				...
				if(textValue.startsWith("upload/"))
				{
					...
					HSSFRichTextString richString = new HSSFRichTextString(
							textValue);
					HSSFFont font3 = workproduct.createFont();
				 ...
				 * workproduct.addPicture( bsValue,
				 * HSSFWorkproduct.PICTURE_TYPE_JPEG)); } else{ //其它数据类型都当作字符串简单处理
