...
<jsp:include page="../header.jsp"></jsp:include>
	<div class="col-md-9 wow fadeInLeft">
		<ul class="breadcrumb">
  			...
  			<li><a href="<%=basePath %>ProductSell/frontlist">农产品预售信息列表</a></li>
  			<li class="active">查询结果显示</li>
  			<a class="pull-right" href="<%=basePath %>ProductSell/productSell_frontAdd.jsp" style="display:none;">添加农产品预售</a>
		</ul>
		<div class="row">
			<%
				/*计算起始序号*/
				...
				for(int i=0;i<productSellList.size();i++) {
            		...
            		ProductSell productSell = productSellList.get(i); //获取到农产品预售对象
            		String clearLeft = "";
            		if(i%4 == 0) clearLeft = "style=\"clear:left;\"";
			%>
			<div class="col-md-3 bottom15" <%=clearLeft %>>
			  <a  href="<%=basePath  %>ProductSell/<%=productSell.getSellId() %>/frontshow"><img class="img-responsive" src="<%=basePath%><%=productSell.getProductPhoto()%>" /></a>
			     <div class="showFields">
			     	 
			     	<div class="field">
	            		农产品名称:<%=productSell.getProductName() %>
			     	</div>
			     	<div class="field">
	            		农产品类别:<%=productSell.getProductClassObj().getProductClassName() %>
			     	</div>
			     	<div class="field">
	            		生产地:<%=productSell.getPublish() %>
			     	</div>
			     	<div class="field">
	            		制造商:<%=productSell.getAuthor() %>
			     	</div>
			     	<div class="field">
	            		出售价格:<%=productSell.getSellPrice() %>
			     	</div>
			     	<div class="field">
	            		新旧程度:<%=productSell.getXjcd() %>
			     	</div>
			     	<div class="field">
	            		发布用户:<%=productSell.getUserObj().getName() %>
			     	</div>
			     	<div class="field">
	            		用户发布时间:<%=productSell.getAddTime() %>
			     	</div>
			        <a class="btn btn-primary top5" href="<%=basePath %>ProductSell/<%=productSell.getSellId() %>/frontshow">详情</a>
			        <a class="btn btn-primary top5" onclick="productSellEdit('<%=productSell.getSellId() %>');" style="display:none;">修改</a>
			        <a class="btn btn-primary top5" onclick="productSellDelete('<%=productSell.getSellId() %>');" style="display:none;">删除</a>
			     </div>
			</div>
			<%  } %>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="productSellQueryForm" id="productSellQueryForm" action="<%=basePath %>ProductSell/frontlist" class="mar_t15">
  ...
  <div class="modal-dialog" style="width:900px;" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title"><i class="fa fa-edit"></i>&nbsp;农产品预售信息编辑</h4>
      </div>
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 <label for="productSell_productPhoto_edit" class="col-md-3 text-right">农产品主图:</label>
		  	 <div class="col-md-9">
		  ...
		  <div class="form-group">
		  	 <label for="productSell_productName_edit" class="col-md-3 text-right">农产品名称:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="productSell_productName_edit" name="productSell.productName" class="form-control" placeholder="请输入农产品名称">
			 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="productSell_productClassObj_productClassId_edit" class="col-md-3 text-right">农产品类别:</label>
		  	 <div class="col-md-9">
		  ...
		  <div class="form-group">
		  	 <label for="productSell_publish_edit" class="col-md-3 text-right">生产地:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="productSell_publish_edit" name="productSell.publish" class="form-control" placeholder="请输入生产地">
			 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="productSell_author_edit" class="col-md-3 text-right">制造商:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="productSell_author_edit" name="productSell.author" class="form-control" placeholder="请输入制造商">
			 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="productSell_xjcd_edit" class="col-md-3 text-right">新旧程度:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="productSell_xjcd_edit" name="productSell.xjcd" class="form-control" placeholder="请输入新旧程度">
			 </div>
		  </div>
		  <div class="form-group">
		  	 <label for="productSell_sellDesc_edit" class="col-md-3 text-right">出售说明:</label>
		  	 <div class="col-md-9">
			 	<textarea name="productSell.sellDesc" id="productSell_sellDesc_edit" style="width:100%;height:500px;"></textarea>
			 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group">
		  	 <label for="productSell_addTime_edit" class="col-md-3 text-right">用户发布时间:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="productSell_addTime_edit" name="productSell.addTime" class="form-control" placeholder="请输入用户发布时间">
			 </div>
		  </div>
		</form> 
...
<script>
//实例化编辑器
var productSell_sellDesc_edit = UE.getEditor('productSell_sellDesc_edit'); //出售说明编辑器
var basePath = "<%=basePath%>";
/*跳转到查询结果的某页*/
function GoToPage(currentPage,totalPage) {
    if(currentPage==0) return;
    if(currentPage>totalPage) return;
    document.productSellQueryForm.currentPage.value = currentPage;
    document.productSellQueryForm.submit();
}

/*可以直接跳转到某页*/
function changepage(totalPage)
{
    var pageValue=document.productSellQueryForm.pageValue.value;
    if(pageValue>totalPage) {
        alert('你输入的页码超出了总页数!');
        return ;
    }
    document.productSellQueryForm.currentPage.value = pageValue;
    documentproductSellQueryForm.submit();
}

/*弹出修改农产品预售界面并初始化数据*/
function productSellEdit(sellId) {
	$.ajax({
		url :  basePath + "ProductSell/" + sellId + "/update",
		type : "get",
		dataType: "json",
		success : function (productSell, response, status) {
			if (productSell) {
				...
				$("#productSell_publish_edit").val(productSell.publish);
				$("#productSell_author_edit").val(productSell.author);
				$("#productSell_sellPrice_edit").val(productSell.sellPrice);
				$("#productSell_xjcd_edit").val(productSell.xjcd);
				productSell_sellDesc_edit.setContent(productSell.sellDesc, false);
				$.ajax({
...
function productSellDelete(sellId) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "ProductSell/deletes",
			data : {
				sellIds : sellId,
			},
			success : function (obj) {
				if (obj.success) {
					alert("删除成功");
					$("#productSellQueryForm").submit();
					//location.href= basePath + "ProductSell/frontlist";
				}
				else 
					alert(obj.message);
			},
...
/*ajax方式提交农产品预售信息表单给服务器端修改*/
function ajaxProductSellModify() {
	$.ajax({
		url :  basePath + "ProductSell/" + $("#productSell_sellId_edit").val() + "/update",
		type : "post",
		dataType: "json",
